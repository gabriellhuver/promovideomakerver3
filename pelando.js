...
exports.fetchData = async function (client) {
    return new Promise(async (resolve, reject) => {
        try {
            console.log('Buscando pelos melhores produtos no pelando.com')
            try {
                client.emit('message', 'Buscando pelos melhores produtos no pelando.com')
            } catch (error) { }
            var prodList = []
                ...
                for (let index = 0; index < res.length; index++) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            try {
                                client.emit('message', `Url de produto achada ${product.url}`)
                            } catch (error) { }
